...
The extent of data transformation required to achieve anonymization remains the subject of significant debate within the compliance community. The Digital Omnibus~\cite{digital_omnibus} in the European Union (EU) has once again brought the distinction between personal and non-personal data to the forefront of regulatory debate. Such debate suggests not only that the categorisation of data continues to present significant operational challenges, but also that the purpose-oriented dimension embedded in data protection law remains insufficiently understood.
...
% Left children: position so their east anchor sits to the LEFT of the shared spine
\node[anchor=east, font=\scriptsize\ttfamily] (lkind)    at ($(root.south west) + (-0.35, -0.35)$) {:Kind};
\node[anchor=east, font=\scriptsize\ttfamily] (ldomain)  at ($(lkind.east) + (0, -0.35)$) {:Domain};
\node[anchor=east, font=\scriptsize\ttfamily] (lcontrol) at ($(ldomain.east) + (0, -0.35)$) {:Control};
\node[anchor=east, font=\scriptsize\ttfamily] (lstat)    at ($(lcontrol.east) + (0, -0.35)$) {:Statistical};

% Edges from the same shared spine ([xshift=10pt]root.south west) going LEFT to each left child
\draw ([xshift=10pt]root.south west) |- (lkind.east);
\draw ([xshift=10pt]root.south west) |- (ldomain.east);
\draw ([xshift=10pt]root.south west) |- (lcontrol.east);
\draw ([xshift=10pt]root.south west) |- (lstat.east);
\end{tikzpicture}
\end{minipage}%
\hspace{0.5em}%
...
The Italian DPA applies a stricter GDPR interpretation, treating \emph{any} unique information about individuals as personal data. While GDPR requires both cardinality and knowability for identifiability, the Italian DPA considers uniqueness alone sufficient. Thus internal UUIDs with \clabel{ClosedKnowability} become \clabel{PersonalData} under Italian rules.
